...
module RelCats-quantale {ro rl re} (RC : Category ro rl re) where 

module RC = Category RC 
...
record RelObj : Set (ro ⊔ lsuc lzero) where
  constructor R[_,_,_]
  field
    fst : Set 
    snd : Set 
...
preservesRel : ∀ {A B A' B' : Set} → (R : REL0 A B) → (R' : REL0 A' B')
               → (f : A → A') → (g : B → B') → Set _
preservesRel R R' f g = ∀ {x y} → (R x y) RC.⇒ (R' (f x) (g y)) 
...
preservesRel-comp R R' R'' f g f' g' pfg pf'g' = RC [ pf'g' ∘ pfg   ] 


preservesRelObj : ∀ (R R' : RelObj)
               → (f : RelObj.fst R → RelObj.fst R') → (g : RelObj.snd R → RelObj.snd R') → Set _
...
preservesRelObj-comp R R' R'' f g f' g' pfg pf'g' = RC [ pf'g' ∘ pfg ] 




record RelMorph (R S : RelObj) : Set rl where
...
idRelMorph = record { mfst = idf ; msnd = idf ; preserves = RC.id } 
...
Rels = record
  ...
  ; _∘_ = λ f g → record { mfst = mfst f Sets.∘ mfst g ; msnd = msnd f Sets.∘ msnd g
                         ; preserves = RC [  preserves f ∘ preserves g ] }
...
Rels^ : ℕ → Category _ _ _ 
Rels^ k = Cat^ Rels k

[Rels^_,Rels] : ∀ k → Category _ _ _ 
...
record RTObj (k : ℕ) : Set (ro ⊔ rl ⊔ lsuc lzero) where 
  open RelObj
  field 
...
record RTMorph {k : ℕ} (H K : RTObj k) : Set (lsuc lzero ⊔ ro ⊔ rl) where 
...
idRTMorph {k} H = record { d1 = idnat ; d2 = idnat ; d-resp = id-resp  } 
          where id-resp : ∀ (Rs : Vec RelObj k) → RTMorph-resp H H idnat idnat Rs
                id-resp Rs {x} {y} rewrite get-comp-1-id H Rs {x} | get-comp-2-id H Rs {y} = RC.id
...
{- we can define the rest if our relation category supports sums and products 
