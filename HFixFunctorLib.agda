...
module object-fixpoint {k : ℕ} (H : Functor ([Sets^ k ,Sets]) ([Sets^ k ,Sets])) where 

    {-# NO_POSITIVITY_CHECK #-}
    data HFixFunctor : Vec Set k → Set 


    {-# TERMINATING #-}
    HFix-fmap : ∀ {As Bs : Vec Set k} → VecFSpace As Bs → HFixFunctor As → HFixFunctor Bs


    {-# TERMINATING #-}
    HFix-id : ∀ {As} {xs} → HFix-fmap {As} {As} (Category.id (Sets^ k )) xs ≡ xs

    {-# TERMINATING #-}
    HFix-homo : ∀ {As Bs Cs} → {f : VecFSpace As Bs} → {g : VecFSpace Bs Cs}
                → ∀ {x} → HFix-fmap (Category._∘_ (Sets^ k )  g f) x ≡ HFix-fmap g (HFix-fmap f x)

    {-# TERMINATING #-}
    HFix-resp : ∀ {As Bs} → {fs gs : VecFSpace As Bs}
                → (Sets^ k) [ fs ≈  gs ]
                → ∀ {x : HFixFunctor As} → HFix-fmap fs x ≡ HFix-fmap gs x

    -- compute (object) fixpoint of higher order functor H 
    fixHFunc : Functor (Sets^ k) Sets
    fixHFunc = record
                { F₀ = HFixFunctor 
                ; F₁ = HFix-fmap 
                ; identity = HFix-id 
                ; homomorphism = HFix-homo 
                ; F-resp-≈ = HFix-resp 
                }

    data HFixFunctor where
        hffin : ∀ {As : Vec Set k}
                → Functor.F₀ (Functor.F₀ H fixHFunc) As 
                → HFixFunctor As

    HFix-fmap {As} {Bs} gs (hffin {As} x) = hffin (Functor.F₁ (Functor.F₀ H (fixHFunc)) gs x)

    HFix-id {As} {hffin x} = ≡.cong hffin (Functor.identity  (Functor.F₀ H (fixHFunc)))

    HFix-homo {As} {Bs} {Cs} {f} {g} {hffin x} = ≡.cong hffin (Functor.homomorphism (Functor.F₀ H (fixHFunc)))

    HFix-resp {As} {Bs} {f} {g} f≡g {hffin x} = ≡.cong hffin (Functor.F-resp-≈ (Functor.F₀ H (fixHFunc)) f≡g)
...
module morphism-fixpoint where 
  ...
  mutual 
      {-# TERMINATING #-}
      HFix-hmap-component : ∀ {k} {H1 H2 : Functor ([Sets^ k ,Sets]) ([Sets^ k ,Sets])}
                          → NaturalTransformation H1 H2 
                          → (Xs : Vec Set k) → HFixFunctor H1 Xs → HFixFunctor H2 Xs
      HFix-hmap-component {k} {H1} {H2} η Xs (hffin x) = 
              let μη = HFix-hmap H1 H2 η -- n.t. from fixHFunc H1 to fixHFunc H2
      ...
      HFix-hmap {k} H1 H2 η = record { η = hη ; commute = commutes ; sym-commute = λ f → ≡.sym (commutes f)  } 
          where
                hη : (Xs : Vec Set k) → HFixFunctor H1 Xs → HFixFunctor H2 Xs
                hη = HFix-hmap-component η 
                {-
                  -- need natural transformation from (Functor.F₀ H1 (fixHFunc H1)) to (Functor.F₀ H2 (fixHFunc H2))
                  -- (H1 (fixHFunc H1)) Xs ------> (H1 (fixHFunc H2)) s
                  --        |                               |
                  --        v                               v
                ...
                commutes : ∀ {Xs Ys : Vec Set k} → (f : VecFSpace Xs Ys)
                              → ∀ {x : HFixFunctor H1 Xs}
                              → hη Ys (Functor.F₁ (fixHFunc H1) f x) ≡ Functor.F₁ (fixHFunc H2) f (hη Xs x)
